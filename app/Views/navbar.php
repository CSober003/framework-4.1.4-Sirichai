  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
    ...
    <form class="navbar-nav mr-auto mt-2 mt-lg-0">
        ...
        <?php }else { ?>
              ...
              <a class="nav-link" onclick="alertmsg();" style="cursor: pointer;">ลงทะเบียน</a>
        <?php } ?>
        <script>
          function alertmsg() {
    ...
    <form class="form-inline my-2 my-lg-0">
      <?php $checkname = $session->get('name');
      if(isset($checkname)) {?>
        <a href="/username" class="pr-4" style="color:white; text-decoration:none;"><?php echo $session->get('name')?></a>
        <a href="/Logout/logout" class="btn btn-danger my-2 my-sm-0">ออกจากระบบ</a>
      <?php }else { ?>
        <a href="/login" class="btn btn-success my-2 my-sm-0 mr-3">เข้าสู่ระบบ</a>
        <a href="/register" class="btn btn-light my-2 my-sm-0">สมัครสมาชิก</a>
      <?php } ?>
    </form>
  </div>
</nav>
